...
import os
from method_computation import utilities, constants, method, call

output_path = "method_computation\\Methods"

print ("SOUPIFYING...")
soup = BeautifulSoup (open ("allmeths.xml", mode = "r", encoding = "utf-8").read (), "html.parser")
...
for p in meths:
    ...
    # WRITE FILE
    file = open (path_level_4, "w", encoding = "utf-8")
    for i in sorted (method_args.keys ()):
        try:
            file.write (i + "|" + method_args [i] + "\n")
        except UnicodeEncodeError:
            print (method_args [i].encode ("utf-8"))
            stop
    file.close ()
...
print (str (files_done) + " \t100%")

index_file = open (os.path.join (output_path, "index.txt"), "w", encoding = "utf-8")
