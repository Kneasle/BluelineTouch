...
class Touch:
	def __init__ (self, methods, calls = "", method_calls = (0, [(0, 0)]), target_change = constants.ROUNDS,
				  calling_type = constants.CALL_SEQUENCE, bell_calling_from = constants.TENOR,
				  automatically_compute_changes = False, change_checks = constants.ALL_CHANGES):
		...
		if bell_calling_from == constants.TENOR:
			self.bell_calling_from = self.stage - 1
		if target_change is None:
			self.target_change = change.rounds (self.stage)
	...
	def update_calls_from_string (self, calls, call_type = None):
		...
		self.calls = []
		if self.calling_type == constants.CALL_SEQUENCE:
			for i in range (len (calls)):
				call = calls [i].upper ()

				# CONVERT CALLS THAT NEED TRANSLATING, e.g. "-" => "B", "P" => "M"
				if call in constants.touch_call_conversions:
					call = constants.touch_call_conversions [call]
		...
		elif self.calling_type == constants.CALLING_POSITIONS:
			call_name = "B"
			for i in range (len (calls)):
				call = calls [i].upper ()
				...
				if not has_found_ligit_position:  # Try converting the calling positions
					converted_call = call
					if converted_call in constants.calling_position_conversions.keys ():
						converted_call = constants.calling_position_conversions [converted_call]
				...
				if has_found_ligit_position:
					...
					corrected_call = call
					if corrected_call in constants.touch_call_conversions.keys ():
						corrected_call = constants.touch_call_conversions [corrected_call]
		...
		elif self.calling_type == constants.BELLS_BEFORE:
			call_name = "B"
			lookup_string = str (change.rounds (self.stage))
