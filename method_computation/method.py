...
class Method:
    def __init__ (self, stage = 8, name = None, classification = None, place_notation = "x18x18x18x18,12",
                  calls = STANDARD, title = None, **kwargs):
        self.stage = stage
        self.name = name
        ...
        self.lookup_table = generate_look_up_table (self.expanded_notation, self.stage)
        # endregion
        ...
        if name is None:
            ...
            if len (parts) > 2:
                ...
                if self.hunt_bells == []:
                    ...
                    self.classification = PRINCIPLE
                else:
                    self.classification = BOB
        
        if title is None:
            if self.classification is PRINCIPLE:
                self.title = self.name + " " + all_stages [self.stage]
            else:
                self.title = self.name + " " + self.classification + " " + all_stages [self.stage]
        # endregion
        
        # region COMPUTE CALL DATA
        if self.calls == STANDARD:
            self.calls = generate_standard_bobs_and_singles (self)
        elif type (self.calls) is str:
            self.calls = calls_from_string (self.calls)
        elif type (self.calls) is dict:
            self.calls = [Call (i, self.calls [i]) for i in self.calls.keys ()]
    ...
    def generate_plain_lead (self):
        changes = []
        current_change = rounds (self.stage)
